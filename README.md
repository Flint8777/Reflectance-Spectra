...
- 図の書き出し（SVG ベクター / PNG 2x・4x）— L 字の軸・グリッド無し・副目盛り付き、凡例はスペクトルに重ならない位置へ自動配置
...
| ファイルから開く | `.dpt` と OPUS の数字拡張子（`.0`〜`.9`）はダブルクリックで開ける（インストーラ版のみ）。起動済みなら同じウィンドウに追加される |
...
| 図の書き出し | ツールバーの **Export figure** ボタン → SVG（ベクター）または PNG（2x / 4x）。凡例は書き出す図にだけ入り、重ならない位置に自動で置かれる |
...
> **実装メモ**: [brukeropus](https://github.com/joshduran/brukeropus) (MIT) のロジックを参考に JavaScript に移植。`src/opusParser.js` を参照。
