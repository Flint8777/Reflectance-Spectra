...
describe('scaleToUnitInRange', () => {
    ...
    it('xRange が null のとき全範囲でスケール', () => {
        expect(scaleToUnitInRange([1, 2, 3], [10, 20, 30], null)).toEqual([
            0, 0.5, 1,
        ]);
    });
    ...
    it('範囲内にデータ点が無い場合は null', () => {
        expect(
            scaleToUnitInRange([1, 2, 3], [10, 20, 30], [10, 20]),
        ).toBeNull();
    });

    it('範囲内の min と max が同じなら 0 の配列を返す', () => {
        expect(scaleToUnitInRange([1, 2, 3], [5, 5, 5], [1, 3])).toEqual([
            0, 0, 0,
        ]);
    });
});
...
describe('normalizeByMaxInRange', () => {
    ...
    it('xRange が null のときは全範囲の最大値で規格化する', () => {
        expect(normalizeByMaxInRange([1, 2, 3], [1, 2, 4], null)).toEqual([
            0.25, 0.5, 1,
        ]);
    });
    ...
    it('範囲内にデータ点が無いときは null を返す', () => {
        expect(
            normalizeByMaxInRange([1, 2, 3], [10, 20, 30], [10, 20]),
        ).toBeNull();
    });

    it('範囲内の最大値が 0 のときはそのまま返す', () => {
        expect(normalizeByMaxInRange([1, 2, 3], [0, 0, 0], [1, 3])).toEqual([
            0, 0, 0,
        ]);
    });
});
