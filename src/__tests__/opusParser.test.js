...
const INFO_BYTES = STRUCT_3D_INFO_TEST.reduce(
    (s, [, f]) => s + (f === 'i' ? 4 : 8),
    0,
);
...
const buildSeriesBlock = (spectraList, infoOverrides = []) => {
    ...
    for (let i = 0; i < num; i++) {
        ...
        for (const [key, fmt] of STRUCT_3D_INFO_TEST) {
            ...
            if (fmt === 'i') {
                view.setInt32(p, v, true);
                p += 4;
            } else {
                view.setFloat64(p, v, true);
                p += 8;
            }
        }
        off += infoSize;
...
const buildOpusFile = (blocks) => {
    ...
    // data blocks
    for (let i = 0; i < padded.length; i++)
        out.set(padded[i].bytes, offsets[i]);
    // directory
...
describe('parseOpusBuffer', () => {
    ...
    it('Series データブロック (type[5]==2) を各 time slice として展開する', () => {
        ...
        // y データ
        expect(result.spectra[0].y).toEqual(
            [0.1, 0.2].map((v) => expect.closeTo(v, 5)),
        );
