...
const parseData = (view, blockStart, blockSize, dpf) => {
    ...
    if (dpf === 2) {
        ...
        for (let i = 0; i < count; i++)
            arr[i] = view.getInt32(blockStart + i * 4, true);
        return arr;
    }
    const arr = new Float32Array(count);
    for (let i = 0; i < count; i++)
        arr[i] = view.getFloat32(blockStart + i * 4, true);
    return arr;
};

// FileBlock 分類ヘルパー（brukeropus block.py 相当）
const isDirectoryBlock = (t) =>
    t[0] === 0 &&
    t[1] === 0 &&
    t[2] === 0 &&
    t[3] === 13 &&
...
const isParamBlock = (t) =>
    ...
    (t[0] === 0 &&
        ...
        t[5] === 1);
const isDataBlock = (t) =>
    t[2] === 0 && t[3] !== 0 && t[3] !== 13 && t[5] !== 2 && t[5] !== 5;
...
const isSeriesData = (t) =>
    t[2] === 0 && t[3] !== 0 && t[3] !== 13 && t[5] === 2;
...
const parseDataSeries = (view, blockStart, blockSize) => {
    ...
    for (let i = 0; i < numBlocks; i++) {
        ...
        for (let j = 0; j < count; j++)
            y[j] = view.getFloat32(blockStart + off + j * 4, true);
        // info block 読み出し
        const info = {};
        let p = blockStart + off + dataSize;
        for (const [key, fmt] of STRUCT_3D_INFO) {
            if (fmt === 'i') {
                info[key] = view.getInt32(p, true);
                p += 4;
            } else {
                info[key] = view.getFloat64(p, true);
                p += 8;
            }
        }
        spectra.push({ y, info });
...
const matchStatusByType = (data, statusBlocks) =>
    statusBlocks.filter(
        (s) =>
            s.type[0] === data.type[0] &&
            s.type[1] === data.type[1] &&
            s.type[3] === data.type[3] &&
            s.type[4] === data.type[4] &&
            s.type[5] === data.type[5],
    );
...
const pairSeriesAndStatus = (seriesBlocks, statusBlocks) => {
    ...
    for (const s of seriesBlocks) {
        ...
        const candidates = matchStatusByType(s, statusBlocks).filter(
            (st) => st.params.npt === exactNpt,
        );
    ...
    for (const s of remaining) {
        const free = matchStatusByType(s, statusBlocks).filter(
            (st) => !used.has(st.start),
        );
...
const pairDataAndStatus = (dataBlocks, statusBlocks) => {
    ...
    const entries = dataBlocks.map((d) => {
        ...
        const candidates = matchStatusByType(d, statusBlocks).filter(
            (s) => typeof s.params.npt === 'number' && count >= s.params.npt,
        );
...
export const parseOpusBuffer = (arrayBuffer) => {
    ...
    const dirBlocks = parseDirectory(
        view,
        header.directoryStart,
        header.numBlocks,
    );
